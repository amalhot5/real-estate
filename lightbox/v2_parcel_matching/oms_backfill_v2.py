...
def load_lightbox_data(fpath: str, service_zips: list) -> pd.DataFrame:
    ...
    df = df[['DEED_LID', 'SITE_ADDR', 'SITE_CITY', 'SITE_ZIP', 'DATE_TRANSFER',
...
def first_round_match(pw_closings, lb_closings, parcel_match):
    first_match = pw_closings[~pw_closings['id'].isin(parcel_match['id'])]
    off_market_sales = lb_closings[~lb_closings['DEED_LID'].isin(parcel_match['DEED_LID'])]
    
    first_match['match_key'] = [x.upper() for x in first_match['address']]
    first_match = first_match.merge(off_market_sales, how='inner', left_on='match_key', right_on='SITE_ADDR')
    first_match = first_match[(abs(first_match['last_event'] - first_match['DATE_TRANSFER']) <= DAY_THRESH)
                              & (abs((first_match['sold_price'] - first_match['VAL_TRANSFER'])/first_match['sold_price']) <= PRICE_THRESH)]
    return first_match[['id', 'DEED_LID']]


def second_round_match(pw_closings, lb_closings, parcel_match, first_match):
    second_match = pw_closings[~(pw_closings['id'].isin(first_match['id'])) & ~(pw_closings.isin(parcel_match['id']))]
    print('second match done filtering')
    second_match['normalized_address'] = [standardize_address(x) for x in tqdm(second_match['address'])]
    off_market_sales = lb_closings[~(lb_closings['DEED_LID'].isin(first_match['DEED_LID'])) & ~(deeds_df['DEED_LID'].isin(parcel_match['DEED_LID']))]
    print('second match done filtering')
    second_match = second_match.merge(off_market_sales, how='inner', left_on='normalized_address', right_on='SITE_ADDR')
    second_match = second_match[(abs((second_match['sold_price'] - second_match['VAL_TRANSFER'])/second_match['sold_price']) <= PRICE_THRESH)
                                & (abs(second_match['last_event'] - second_match['DATE_TRANSFER']) <= DAY_THRESH)]
...
def third_round_match(pw_closings, lb_closings, parcel_match, first_match, second_match):
    off_market_sales = lb_closings[~(lb_closings['DEED_LID'].isin(second_match['DEED_LID'])) & ~(lb_closings['DEED_LID'].isin(first_match['DEED_LID'])) & ~(lb_closings['DEED_LID'].isin(parcel_match['DEED_LID']))]
    third_match = pw_closings[~(pw_closings['id'].isin(first_match['id'])) & ~(pw_closings['id'].isin(second_match['id'])) & ~(pw_closings.isin(parcel_match['id']))]
    third_match['normalized_address'] = [standardize_address(x) for x in tqdm(third_match['address'])]
    ...
    off_market_sales[~pd.isna(off_market_sales['matched_listing_id'])].merge(third_match, how='inner', left_on='matched_listing_id', right_on='id').to_csv('data/output/fuzzy_matches.csv')
...
def parcel_matching(lb_closings: pd.DataFrame, pw_closings: pd.DataFrame) -> pd.DataFrame:
    ...
    return listings_with_oms[(
        abs(listings_with_oms['VAL_TRANSFER'] - listings_with_oms['sold_price'])/listings_with_oms['VAL_TRANSFER'] < PRICE_THRESH)
                                  & (abs(listings_with_oms['DATE_TRANSFER'] - listings_with_oms['last_event']) < DAY_THRESH)]\
            [['id', 'DEED_LID']]
...
# Matching via parcels
parcel_match = parcel_matching(deeds_df, pw_closings)
...
first_match = first_round_match(pw_closings, deeds_df, parcel_match)
print(f"done first match: {len(pw_closings[pw_closings['id'].isin(first_match['id'])])} out of {len(pw_closings[~pw_closings['id'].isin(parcel_match['id'])])}")
print(f"{len(pw_closings[pw_closings['id'].isin(first_match['id'])])/(len(pw_closings[~pw_closings['id'].isin(parcel_match['id'])])) * 100}%")

second_match = second_round_match(pw_closings, deeds_df, parcel_match, first_match)
print(f"done second match: {len(pw_closings[(pw_closings['id'].isin(second_match['id']))])} out of {len(pw_closings[~(pw_closings['id'].isin(first_match['id'])) & ~(pw_closings.isin(parcel_match['id']))])}")
print(f"{len(pw_closings[pw_closings['id'].isin(first_match['id'])])/(len(pw_closings[~(pw_closings['id'].isin(first_match['id'])) & ~(pw_closings.isin(parcel_match['id']))])) * 100}%")


third_match = third_round_match(pw_closings, deeds_df, parcel_match, first_match, second_match)
print(f"num last match: {third_match['matched_listing_id'].nunique()} out of {len(pw_closings[~(pw_closings['id'].isin(first_match['id'])) & ~(pw_closings['id'].isin(second_match['id'])) & ~(pw_closings.isin(parcel_match['id']))])}")
print(f"{len(pw_closings[pw_closings['id'].isin(third_match['matched_listing_id'].nunique())])/(len(pw_closings[~(pw_closings['id'].isin(first_match['id'])) & ~(pw_closings['id'].isin(second_match['id'])) & ~(pw_closings.isin(parcel_match['id']))])) * 100}%")

deeds_df = deeds_df[~((deeds_df['DEED_LID'].isin(parcel_match['DEED_LID']))
                      & (deeds_df['DEED_LID'].isin(first_match['DEED_LID']))
                      & (deeds_df['DEED_LID'].isin(second_match['DEED_LID']))
                      & (deeds_df['DEED_LID'].isin(third_match['DEED_LID'])))]
print(f"total oms: {len(deeds_df)}, unmatched pw listings: {len(pw_closings[~((pw_closings['id'].isin(first_match['id'])) | (pw_closings['id'].isin(second_match['id'])) | (pw_closings['id'].isin(parcel_match['id'])) | (pw_closings['id'].isin(third_match['matched_listing_id'])))])}")
dupe_oms = dedupe(deeds_df)
deeds_df[~(deeds_df['DEED_LID'].isin(dupe_oms))].to_csv('data/output/off_market_sales.csv', index=False)
pw_closings[~((pw_closings['id'].isin(first_match['id'])) 
              | (pw_closings['id'].isin(second_match['id'])) 
              | (pw_closings['id'].isin(third_match['matched_listing_id']))
              | (pw_closings['id'].isin(parcel_match['id'])))].to_csv('data/output/unmatched_mls_listings.csv', index=False)
print(f"total oms: {len(deeds_df[~(deeds_df['DEED_LID'].isin(dupe_oms))])}, unmatched pw listings: {len(pw_closings[~((pw_closings['id'].isin(first_match['id'])) | (pw_closings['id'].isin(second_match['id'])) | (pw_closings['id'].isin(parcel_match['id'])) | (pw_closings['id'].isin(third_match['matched_listing_id'])))])}")