...
def first_round_match(first_match, off_market_sales, day_threshold: tuple, price_threshold: tuple):
    ...
    if price_threshold[1]:
        if day_threshold[1]:
            first_match = first_match[(abs(first_match['last_event'] - first_match['DATE_TRANSFER']) < day_threshold[1])
                              & (abs(first_match['last_event'] - first_match['DATE_TRANSFER']) >= day_threshold[0])
                              & (abs((first_match['sold_price'] - first_match['VAL_TRANSFER'])/first_match['sold_price']) < price_threshold[1])
                              & (abs((first_match['sold_price'] - first_match['VAL_TRANSFER'])/first_match['sold_price']) >= price_threshold[0])]
        else:
        ...
        if day_threshold[1]:
            ...
            first_match = first_match[(abs(first_match['last_event'] - first_match['DATE_TRANSFER']) >= day_threshold[0])
                              & (abs((first_match['sold_price'] - first_match['VAL_TRANSFER'])/first_match['sold_price']) >= price_threshold[0])]
...
def main(day_range: tuple, price_range: tuple):
    ...
    print(f"done first match: {len(pw_closings[pw_closings['id'].isin(first_match['id'])])} out of {len(pw_closings)}")
    print(f"{len(pw_closings[pw_closings['id'].isin(first_match['id'])])/(len(pw_closings)) * 100}%")
