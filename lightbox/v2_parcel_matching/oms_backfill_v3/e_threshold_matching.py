...
def threshold_match(pw_closings: pd.DataFrame, lb_deeds: pd.DataFrame,
    ...
    matched_pw_ids = []
    matched_lb_ids = []
    for pw_zip, pw_price, pw_date, pw_id in tqdm(zip(pw_closings['check_zip'], pw_closings['sold_price'], pw_closings['last_event'], pw_closings['id']), total=len(pw_closings)):
        check = lb_deeds[lb_deeds['SITE_ZIP'] == pw_zip]
        found_lb_id = []
        try:
            for lb_price, lb_date, lb_id in zip(check['VAL_TRANSFER'], check['DATE_TRANSFER'], check['DEED_LID']):
...
def main(day_range: tuple, price_range: tuple) -> None:
    ...
    print(f"done threshold match: {len(pw_closings[(pw_closings['id'].isin(fourth_match['id']))])} out of {len(pw_closings)}")
    print(f"{len(pw_closings[pw_closings['id'].isin(fourth_match['id'])])/(len(pw_closings)) * 100}%")
