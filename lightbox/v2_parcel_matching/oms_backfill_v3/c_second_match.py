...
def second_round_match(second_match, off_market_sales, day_threshold, price_threshold):
    ...
    second_match['normalized_address'] = [standardize_address(x) for x in second_match['address']]
    #second_match['normalized_address'] = second_match['address'].apply(standardize_address(x, suffixes))
    off_market_sales['normalized_address'] = [standardize_address(x) for x in off_market_sales['SITE_ADDR']]
    ...
    if price_threshold[1]:
        if day_threshold[1]:
            second_match = second_match[(abs((second_match['sold_price'] - second_match['VAL_TRANSFER'])/second_match['sold_price']) < price_threshold[1])
                                & (abs((second_match['sold_price'] - second_match['VAL_TRANSFER'])/second_match['sold_price']) >= price_threshold[0])
                                & (abs(second_match['last_event'] - second_match['DATE_TRANSFER']) < day_threshold[1])
                                & (abs(second_match['last_event'] - second_match['DATE_TRANSFER']) >= day_threshold[0])]
        else:
            second_match = second_match[(abs((second_match['sold_price'] - second_match['VAL_TRANSFER'])/second_match['sold_price']) < price_threshold[1])
                                & (abs((second_match['sold_price'] - second_match['VAL_TRANSFER'])/second_match['sold_price']) >= price_threshold[0])
                                & (abs(second_match['last_event'] - second_match['DATE_TRANSFER']) >= day_threshold[0])]
...
def main(day_range: tuple, price_range: tuple):
    ...
    print(f"done second match: {len(pw_closings[(pw_closings['id'].isin(second_match['id']))])} out of {len(pw_closings)}")
    print(f"{len(pw_closings[pw_closings['id'].isin(second_match['id'])])/(len(pw_closings)) * 100}%")
