...
def third_round_match(third_match, off_market_sales, day_threshold, price_threshold):
    third_match["check_zip"] = pd.to_numeric(third_match["zip"], errors="coerce")
    # [int(x.split("-")[0]) if "-" in str(x) else int(x) for x in third_match["zip"]
    # ]

    retlist = []
    for lb_address, lb_price, lb_date, lb_street_name, lb_zip in tqdm(
        zip(
        ...
        found = False
        for pw_address, pw_price, pw_date, l_id in zip(
    ...
    return off_market_sales[~pd.isna(off_market_sales["matched_listing_id"])][
...
def main(day_range: tuple, price_range: tuple, base_fp: str, state: str) -> None:
    ...
    print(
        f"\t\tone third match: {len(pw_closings[(pw_closings['id'].isin(third_match['matched_listing_id']))]['id'].unique())} out of {len(pw_closings['id'].unique())}"
    )
    print(
        f"\t\t{len(pw_closings[pw_closings['id'].isin(third_match['matched_listing_id'])]['id'].unique())/(len(pw_closings['id'].unique())) * 100}%"
    )
    ...
    with open(f"{base_fp}/data/unmatched_closings.pkl", "wb") as f:
        pkl.dump(
            pw_closings[~pw_closings["id"].isin(third_match["matched_listing_id"])], f
        )
    pw_closings[~pw_closings["id"].isin(third_match["matched_listing_id"])].to_csv(
        f"{base_fp}/data/output/unmatched_mls_listings_{day_range[0].days}_{price_range[0]*100:.0f}.csv",
        index=False,
    )
