...
def pull_data(sql_query:str, file_out=None, port_num=2023):
    ...
    engine = create_engine(\
        f"postgresql://teleport:@localhost:{port_num}/pipeline_controller")

    # in order to get around the 300k row limit
    multiplier = 0
    while prev_row_count <= current_row:
        current_row = len(pw_df) + (1000000*multiplier)
        if current_row < 0:
            current_row = 0
        if current_row % 1000000 == 0 and len(pw_df) > 0:
            with open(f'data/prop_pw_df_{multiplier}.pkl', 'wb') as f:
        ...
        try:
            ...
            with engine.connect() as conn:
                ...
                for row in result:
                    pw_df.append(row)
        print(f"{current_row} rows appended, time taken: {time.time() - start}")
...
if __name__ == '__main__':
    sql_query = '''select data ->> 'PropertyType' as property_type
	, data ->> 'PropertySubType' as property_subtype
FROM public.seen_objects
where feed_name = 'ca_crmls_rets' and object_type='LISTING'
order by source_id'''
    pull_data(sql_query)#, 'data/crmls_raw_mls_id.csv')