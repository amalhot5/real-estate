...
def pull_data(sql_query:str, file_out=None, port_num=2023, db_name='public'):
    ...
    while prev_row_count <= current_row:
        current_row = len(pw_df) + (10000*multiplier)
        if current_row < 0:
            current_row = 0
        if current_row % 10000 == 0 and len(pw_df) > 0:
            with open(f'data/normalized_icaar_df_{multiplier}.pkl', 'wb') as f:
        ...
        try:
            ...
            with engine.connect() as conn:
                ...
                for row in result:
                    pw_df.append(row)
        print(f"{len(pw_df) + (10000*multiplier)} rows appended, time taken: {time.time() - start}")
...
if __name__ == '__main__':
    sql_query = '''select source_id,data 
from normalized_objects
where object_type = 'LISTING' and feed_name = 'ia_icaar_rets'
'''
    pull_data(sql_query, db_name='mls_data')#, 'data/crmls_raw_mls_id.csv')